...
Per-run eval is delegated to the shared evaluation/evaluate_reward_model.py script.
...
SHARED_EVAL_ROOT = PROJECT_ROOT.parent / "evaluation"

# --- Training CSV (override: --train_csv) ---
DEFAULT_TRAIN_CSV = SHARED_EVAL_ROOT / "data" / "2026_03_22_low_stakes_training_set_1000_situations_with_CoTs.csv"
...
# --- Dataset alias → shared evaluation CSV path (not flag-overridable) ---
# Mirrors DATASET_ALIASES in evaluation/evaluate_reward_model.py so in-process
# eval reads the same files the subprocess would have resolved.
DATASET_ALIAS_PATHS = {
...
#
# Canonical per-dataset metrics for each run come from the shared
# evaluation/evaluate_reward_model.py script. The script auto-detects
# reward_head.pt inside --model_path. Output lands in
# run_dir/eval_rm_<alias>.json and is the resume sentinel.
# ============================================================

EVAL_SCRIPT_PATH = SHARED_EVAL_ROOT / "evaluate_reward_model.py"
...
def run_script_eval(
    *,
    ...
    if not EVAL_SCRIPT_PATH.exists():
        print(f"[script_eval:{tag}] {EVAL_SCRIPT_PATH} missing; skipping.")
        return None
    ...
    try:
        subprocess.run(cmd, cwd=str(SHARED_EVAL_ROOT), check=True)
